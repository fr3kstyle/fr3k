...
- `ZSHRC` - Shell configuration with FR3K alias
...
3. **Copy configuration:**
   ...
   cp ~/.claude/skills/FR3K/USER/TERMINAL/kitty.conf ~/.config/kitty/
   ```

4. **Add shell aliases:**
   Add the contents of `ZSHRC` to your `~/.zshrc`:
   ```bash
   cat ~/.claude/skills/FR3K/USER/TERMINAL/ZSHRC >> ~/.zshrc
   source ~/.zshrc
   ```
...
The FR3K hook system dynamically updates tab titles and colors to provide real-time visual feedback about what the AI is doing. This creates a glanceable status system across multiple terminal tabs.
...
With multiple terminal tabs running different FR3K sessions, you can instantly see:
