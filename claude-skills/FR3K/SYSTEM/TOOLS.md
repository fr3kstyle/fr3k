# FR3K Tools - CLI Utilities Reference
...
**Location:** `~/.claude/skills/FR3K/Tools/Inference.ts`
...
bun ~/.claude/skills/FR3K/Tools/Inference.ts --level fast "System prompt" "User prompt"

# Standard (Sonnet) - balanced reasoning, typical analysis
bun ~/.claude/skills/FR3K/Tools/Inference.ts --level standard "System prompt" "User prompt"

# Smart (Opus) - deep reasoning, strategic decisions
bun ~/.claude/skills/FR3K/Tools/Inference.ts --level smart "System prompt" "User prompt"

# With JSON output
bun ~/.claude/skills/FR3K/Tools/Inference.ts --json --level fast "Return JSON" "Input"

# Custom timeout
bun ~/.claude/skills/FR3K/Tools/Inference.ts --level standard --timeout 60000 "Prompt" "Input"
```
...
**Programmatic Usage:**
```typescript
import { inference } from '../skills/FR3K/Tools/Inference';
...
**Location:** `~/.claude/skills/FR3K/Tools/RemoveBg.ts`
...
bun ~/.claude/skills/FR3K/Tools/RemoveBg.ts /path/to/image.png

# Remove background and save to different path
bun ~/.claude/skills/FR3K/Tools/RemoveBg.ts /path/to/input.png /path/to/output.png

# Process multiple images
bun ~/.claude/skills/FR3K/Tools/RemoveBg.ts image1.png image2.png image3.png
```
...
**Location:** `~/.claude/skills/FR3K/Tools/AddBg.ts`
...
bun ~/.claude/skills/FR3K/Tools/AddBg.ts /path/to/transparent.png "#EAE9DF" /path/to/output.png

# Add UL brand background color
bun ~/.claude/skills/FR3K/Tools/AddBg.ts /path/to/transparent.png --ul-brand /path/to/output.png
```
...
**Location:** `~/.claude/skills/FR3K/Tools/GetTranscript.ts`
...
bun ~/.claude/skills/FR3K/Tools/GetTranscript.ts "https://www.youtube.com/watch?v=VIDEO_ID"

# Save transcript to file
bun ~/.claude/skills/FR3K/Tools/GetTranscript.ts "https://www.youtube.com/watch?v=VIDEO_ID" --save /path/to/transcript.txt
```
...
**Location:** `~/.claude/skills/FR3K/Tools/extract-transcript.py`
...
**Location:** `~/.claude/skills/FR3K/Tools/YouTubeApi.ts`
...
bun ~/.claude/skills/FR3K/Tools/YouTubeApi.ts --channel-stats

# Get video statistics
bun ~/.claude/skills/FR3K/Tools/YouTubeApi.ts --video-stats VIDEO_ID

# Get latest uploads
bun ~/.claude/skills/FR3K/Tools/YouTubeApi.ts --latest-videos
```
...
1. **Add tool file:** Place `.ts` or `.py` file directly in `~/.claude/skills/FR3K/Tools/`
...
2. **Document here:** Add section to this file with:
   - Tool location (e.g., `~/.claude/skills/FR3K/Tools/ToolName.ts`)
   - Usage examples
   - When to use triggers
   - Environment variables (if any)

3. **Update FR3K/SKILL.md:** Ensure SYSTEM/TOOLS.md is in the documentation index
