...
**The foundational pattern for FR3K extensibility and personalization**

---

## Overview

FR3K uses a consistent two-tier architecture across all configurable components:

```
SYSTEM tier  →  Base functionality, defaults, FR3K updates
USER tier    →  Personal customizations, private policies, overrides
```

This pattern enables:
- **Immediate functionality** — FR3K works out of the box with sensible defaults
- **Personal customization** — Users can override any default without modifying core files
- **Clean updates** — FR3K updates don't overwrite personal configurations
- **Privacy separation** — USER content is never synced to the public FR3K repository

---

## The Lookup Pattern

When FR3K needs configuration, it follows a cascading lookup:
...
```
skills/FR3K/SYSTEM/PAISECURITYSYSTEM/  # SYSTEM tier (base)
├── README.md                          # Overview
...
The SecurityValidator hook checks `USER/PAISECURITYSYSTEM/patterns.yaml` first, falling back to `skills/FR3K/SYSTEM/PAISECURITYSYSTEM/patterns.example.yaml`.

### Response Format

```
skills/FR3K/SYSTEM/RESPONSEFORMAT.md  # SYSTEM tier (base format rules)
skills/FR3K/USER/RESPONSEFORMAT.md    # USER tier (personal overrides)
```
...
Private skills use the `_ALLCAPS` prefix and are never synced to public FR3K.
...
The SYSTEM tier must always provide functional defaults. A fresh FR3K installation should work immediately without requiring USER configuration.
...
The `USER/` directory is excluded from public FR3K sync. Anything in USER:
- Never appears in public FR3K repository
...
When FR3K updates, only SYSTEM tier files change. Your USER configurations remain untouched. This means:
- Safe to update FR3K without losing customizations
...
Future FR3K versions may support declarative merging.
...
- `SYSTEM/PAISYSTEMARCHITECTURE.md` — Overall FR3K architecture
