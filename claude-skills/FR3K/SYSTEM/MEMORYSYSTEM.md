...
This is the actual "firehose" - every message, tool call, and response. FR3K leverages this native storage rather than duplicating it.
...
The `ISC.json` file captures the Ideal State Criteria from FR3K Algorithm execution. This enables:
...
**Structure:**
- `LEARNING/SYSTEM/YYYY-MM/` - FR3K/tooling learnings (infrastructure issues)
...
bun run ~/.claude/skills/FR3K/Tools/FailureCapture.ts --migrate
```
...
bun run ~/.claude/skills/FR3K/Tools/SessionHarvester.ts --recent 10

# Generate pattern synthesis
bun run ~/.claude/skills/FR3K/Tools/LearningPatternSynthesis.ts --week
```
