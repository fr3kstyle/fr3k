...
The FR3K hook system is an event-driven automation infrastructure built on Claude Code's native hook support. Hooks are executable scripts (TypeScript/Python) that run automatically in response to specific events during Claude Code sessions.
...
**Use Cases:**
- Load FR3K context from `skills/FR3K/SKILL.md`
...
**What They Do:**
- `LoadContext.hook.ts` - Reads `skills/FR3K/SKILL.md` and injects FR3K context as `<system-reminder>` at session start
...
- **Inference:** `import { inference } from '../skills/FR3K/Tools/Inference'` → `inference({ level: 'standard', expectJson: true })`
...
- **Inference:** `import { inference } from '../skills/FR3K/Tools/Inference'` → `inference({ level: 'fast' })`
...
- After receiving Task result, parent calls: `bun ~/.claude/skills/FR3K/Tools/UpdateTabTitle.ts "Message"`
- This avoids transcript parsing race conditions
- See `~/.claude/skills/FR3K/Tools/UpdateTabTitle.ts` for implementation
...
**Key Variables:**
- `PAI_DIR` - FR3K installation directory (typically `~/.claude`)
...
- Central to the FR3K install wizard
...
await sendEventToObservability({
  source_app: getSourceApp(),           // 'FR3K' or agent name
...
**Full Documentation:** See `~/.claude/skills/FR3K/SYSTEM/TERMINALTABS.md`
...
2. Is voice_id correct? See `skills/FR3K/SKILL.md` for mappings
...
1. **Added CAPTURE field to response format** (see `~/.claude/skills/FR3K/SKILL.md`)
...
**Tracking:** Documented in `~/.claude/skills/FR3K/SKILL.md` (History Capture System section)
...
**Check:**
1. Does `~/.claude/skills/FR3K/SKILL.md` exist?
...
- **History/Memory:** `~/.claude/skills/FR3K/SYSTEM/MEMORYSYSTEM.md`
...
Path: ~/.claude/skills/FR3K/Tools/Inference.ts
Import: import { inference } from '../skills/FR3K/Tools/Inference'
...
LoadContext.hook.ts             FR3K context loading
...
### `skills/FR3K/Tools/Inference.ts`
