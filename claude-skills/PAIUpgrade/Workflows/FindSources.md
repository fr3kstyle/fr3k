...
This workflow helps identify new sources worth monitoring for FR3K-relevant updates:
...
- Content relevance to FR3K infrastructure
...
**URL:** [url]
**Relevance:** [Why this matters for FR3K]
...
Edit `~/.claude/skills/FR3K/USER/SKILLCUSTOMIZATIONS/PAIUpgrade/youtube-channels.json`

### For Other Sources:
Currently, non-YouTube sources are monitored via the base `sources.json`.
To request additions to base Anthropic monitoring, note them for next FR3K release.
```
...
cat ~/.claude/skills/FR3K/USER/SKILLCUSTOMIZATIONS/PAIUpgrade/youtube-channels.json
