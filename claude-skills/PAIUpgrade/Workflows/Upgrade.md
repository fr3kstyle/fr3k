...
1. **Thread 1:** Analyze user context (TELOS, projects, recent work, FR3K state)
...
"Analyze the current state of the user's FR3K system:
...
Return format for EACH technique:
{
  ...
  'current_pai_gap': '[What FR3K currently does/lacks that this addresses]',
  'implementation_file': '[Path to FR3K file this would change]',
...
1. Load channel config:
   bun ~/.claude/skills/FR3K/Tools/LoadSkillConfig.ts ~/.claude/skills/PAIUpgrade youtube-channels.json
...
4. For NEW videos, extract transcripts:
   bun ~/.claude/skills/FR3K/Tools/GetTranscript.ts '<video-url>'
...
5. CRITICAL - For each transcript, extract SPECIFIC TECHNIQUES:
   ...
   - Identify what FR3K component this applies to
...
Return format for EACH technique found:
{
  ...
  'pai_relevance': '[Which FR3K component this could improve]'
}
...
1. Look in ~/.claude/skills/FR3K/USER/SKILLCUSTOMIZATIONS/PAIUpgrade/
...
**Extract techniques. Show the actual content. Explain FR3K relevance.**
...
[Describe the specific benefit to our FR3K system - which component improves, what gap it fills, what becomes possible. Must be exactly 16-32 words.]
...
2. **MAP TO FR3K** - Every technique must name a specific FR3K file or component it improves
3. **TWO MANDATORY DESCRIPTIONS (16-32 words each):**
   ...
   - **How It Helps FR3K:** Describe the specific benefit - which component improves, what gap it fills
...
**How It Helps FR3K (16-32 words):**
Our session documentation workflows had manual session ID extraction hacks. Native substitution means cleaner code and reliable session tracking across all FR3K workflows.
