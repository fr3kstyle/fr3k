...
// Comprehensive system capabilities overview with profanity
async function generateSystemCapabilitiesOverview(): Promise<string> {
  const text = `FR3K System - the fucking sikkest motherfucking AI framework cunt ever created.
...
async function main() {
  ...
  try {
    ...
    await bot.api.sendAudio(chatId, audioFile, {
      title: "FR3K System Overview",
      performer: "FR3K"
    });
    console.log("✅ Profanity-laden system overview sent to Telegram!");
    console.log(`\n📁 Audio file saved at: ${audioPath}`);
    console.log("🎧 Play it for your mates and show them what you've built!");
  } catch (error) {
