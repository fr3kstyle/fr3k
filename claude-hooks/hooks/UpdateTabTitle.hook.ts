...
function getRunningAgentCount(): number {
  try {
    ...
    const result = execSync(
      'ls /tmp/claude-1000/-home-fr3k/tasks/*.output 2>/dev/null | wc -l',
...
/**
 * Get proactive task status from HEARTBEAT.md
 */
function getProactiveTaskStatus(): string {
  try {
    const { readFileSync, existsSync } = require('fs');
    const heartbeatPath = process.env.HOME + '/.pai-workspace/HEARTBEAT.md';
    if (!existsSync(heartbeatPath)) return '0 tasks';
 ...
 */
function checkConnection(service: 'telegram' | 'voice'): '🟢' | '🔴' {
  try {
    const { execSync } = require('child_process');
    if (service === 'telegram') {
      // Check if pai-telegram-bot service is running
      const result = execSync(
        'systemctl --user is-active pai-telegram-bot.service 2>/dev/null',
        { encoding: 'utf-8', stdio: ['pipe', 'pipe', 'ignore'] }
      );
      ...
      const result = execSync(
        'curl -s http://localhost:8888/health 2>/dev/null | grep -q "healthy"',
 ...
 */
function buildStatusIndicator(): string {
  const agentCount = getRunningAgentCount();
  const proactiveStatus = getProactiveTaskStatus();
  const telegramStatus = checkConnection('telegram');
  const voiceStatus = checkConnection('voice');

  if (agentCount > 0) {
    return `🤖 ${agentCount} | ${proactiveStatus} tasks | ${telegramStatus} TG | ${voiceStatus} Voice | `;
  } else {
    return `${proactiveStatus} | ${telegramStatus} TG | ${voiceStatus} Voice | `;
  }
}
...
function setTabTitle(title: string, state: TabState = 'normal'): void {
  try {
    // Build status indicator with agent count, tasks, and connections
    const statusIndicator = buildStatusIndicator();

    // Add "…" suffix for active states
    const titleWithSuffix = state !== 'normal' ? `${title}…` : title;
    const fullTitle = `${statusIndicator}${titleWithSuffix}`;
    const truncated = fullTitle.length > 80 ? fullTitle.slice(0, 77) + '…' : fullTitle;
