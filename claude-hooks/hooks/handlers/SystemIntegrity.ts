...
import type { ParsedTranscript } from '../../skills/FR3K/Tools/TranscriptParser';
...
const INTEGRITY_SCRIPT = paiPath('skills', 'FR3K', 'Tools', 'IntegrityMaintenance.ts');

/**
 * Send voice notification for integrity check start.
 * Fire-and-forget - doesn't block.
 * Uses ElevenLabs TTS via voice server.
...
async function notifyIntegrityStart(): Promise<void> {
  try {
    ...
    // Load identity for voice settings
...
function spawnIntegrityMaintenance(
  ...
  try {
    ...
    const child = spawn('/home/fr3k/.bun/bin/bun', [INTEGRITY_SCRIPT], {
      detached: true,
