...
(async () => {
  try {
    ...
    // Run the banner tool
    const bannerPath = join(paiDir, 'skills/FR3K/Tools/Banner.ts');
    const result = spawnSync('bun', ['run', bannerPath], {
      encoding: 'utf-8',
      stdio: ['inherit', 'pipe', 'pipe'],
      shell: '/usr/bin/bash',  // Fix ENOENT error
      env: {
        ...process.env,
    ...
    if (isKitty) {
      try {
        execSync(`kitty @ set-tab-title "Ready to work..."`, { stdio: 'ignore', timeout: 2000, shell: '/usr/bin/bash' });
      } catch {
