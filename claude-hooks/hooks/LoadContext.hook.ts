 ...
 * - Files: skills/FR3K/SKILL.md, skills/FR3K/SYSTEM/AISTEERINGRULES.md,
 *          skills/FR3K/USER/AISTEERINGRULES.md, MEMORY/STATE/progress/*.json
 *
 * OUTPUT:
...
function loadContextFiles(paiDir: string, settings: Settings): string {
  const defaultFiles = [
    'skills/FR3K/SKILL.md',
    'skills/FR3K/SYSTEM/AISTEERINGRULES.md',
    'skills/FR3K/USER/AISTEERINGRULES.md'
  ];
...
function loadRelationshipContext(paiDir: string): string | null {
  ...
  const opinionsPath = join(paiDir, 'skills/FR3K/USER/OPINIONS.md');
...
async function checkActiveProgress(paiDir: string): Promise<string | null> {
  ...
  try {
    ...
    summary += '\n💡 To resume: `bun run ~/.claude/skills/FR3K/Tools/SessionProgress.ts resume <project>`\n';
    summary += '💡 To complete: `bun run ~/.claude/skills/FR3K/Tools/SessionProgress.ts complete <project>`\n';
...
async function main() {
  try {
    ...
    const skillMdPath = join(paiDir, 'skills/FR3K/SKILL.md');
    const componentsDir = join(paiDir, 'skills/FR3K/Components');
    ...
    if (needsRebuild) {
      ...
      try {
        execSync('bun ~/.claude/skills/FR3K/Tools/CreateDynamicCore.ts', {
          cwd: paiDir,
          stdio: 'pipe',
          timeout: 5000
        });
